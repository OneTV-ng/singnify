...
import { Play, Music, Album, List, Heart, FileText, AlertCircle, Pause, SkipBack, Share, SkipForward, Volume2, Shuffle, Repeat, User, Users, Plus, Search, Edit } from 'lucide-react';
...
interface LikesTabProps {
  likedItems: (TrackItem[] | LikedItem[]|any[]);
  loading: boolean;
}
interface Following {
  id: string;
...
const FanlinkTab = ({ fanlink, loading }: { fanlink: any[]; loading: boolean }) => {
...
console.log(fanlink);
  ...
  if (fanlink.length === 0) {
    return (
      <div className="text-center py-16">
        <List className="mx-auto mb-4 text-gray-500" size={48} />
        <h3 className="text-xl font-medium text-gray-300 mb-2">No Fanlink found</h3>
        <p className="text-gray-500 mb-6">Generate your fan links</p>
        <button
          onClick={() => router.push('/artist/fanlinks/create')}
          className="bg-purple-600 hover:bg-purple-700 px-6 py-3 rounded-lg font-medium transition-colors flex items-center gap-2 mx-auto"
        >
          <Plus size={20} />
          Fenerate Fanlink
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-white">Playlists ({fanlink.length})</h2>
        <button
          onClick={() => router.push('/artist/playlists/create')}
          className="bg-purple-600 hover:bg-purple-700 px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2"
        >
          <Plus size={20} />
          New Playlist
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {fanlink.map((data) => {
          const artistSlug = slugify(data.artist_name);
          const trackSlug = slugify(data.track_name);
          const link = `/links/${artistSlug}__${trackSlug}`;
          ...
          return (
            ...
            >
              <div className="aspect-square mb-4 rounded-lg overflow-hidden bg-gray-700 relative">
                <img 
                  src={data.image || '/api/placeholder/300/300'} 
                  alt={data.track_name}
                  className="w-full h-full object-cover"
                />
                <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
...
// Likes Component
const LikesTabxx = ({ likedItems, loading }: {
...
const LikesTaboo = ({ likedItems, loading }: LikesTabProps) => {
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-500"></div>
  ...
  if (!likedItems || likedItems.length === 0) {
    return (
      <div className="text-center py-16">
        <Heart className="mx-auto mb-4 text-gray-500" size={48} />
        <h3 className="text-xl font-medium text-gray-300 mb-2">No liked tracks yet</h3>
        <p className="text-gray-500">Your liked songs will appear here once you add some.</p>
      </div>
    );
  }
  ...
  return (
    <>
      <h2 className="text-2xl font-bold text-white mb-6">
        Liked Tracks ({likedItems.length})
      </h2>

      <div className="space-y-4">
        {likedItems.map((track) => (
          <div
            key={track.id}
            className="bg-gray-800 rounded-lg p-4 hover:bg-gray-700 transition-colors cursor-pointer"
            onClick={() => window.location.href = `/artist/song/${track.music_id}`}
          >
            <div className="flex items-center gap-3">
              <div className="w-14 h-14 rounded bg-gray-700 overflow-hidden flex-shrink-0">
                <img
                  src={track.image || "/api/placeholder/48/48"}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="px-2 py-1 rounded-full text-xs bg-blue-600 text-blue-100">
                    Track
                  </span>
                  <h3 className="font-medium text-white truncate">
                    {track.track_name}
                ...
                <p className="text-sm text-gray-400 truncate">
                  {track.base_name} • {track.label} • {track.duration}
                </p>
              </div>
...
const LikesTab = ({ likedItems, loading }: LikesTabProps) => {
...
const ArtistContentPage = () => {
  ...
  const [likedItems, setLikedItems] = useState<LikedItem[]|TrackItem[]>([]);
  const [following, setFollowing] = useState<Following[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState('songs');
  const [fanlink, setFanlink] = useState<any[]>([]);
  // Audio Player State
  ...
  // Parse URL parameters and hash to set initial tab
  useEffect(() => {
    const parseTabFromUrl = () => {
      // Check for hash-based routing (e.g., #albums)
      const hash = window.location.hash.replace('#', '').toLowerCase();
      if (hash && ['songs', 'albums', 'playlists', 'likes', 'fanlink', 'following'].includes(hash)) {
        return hash;
      }

      // Check for query parameter (e.g., ?tab=albums or ?@tab=albums)
      const urlParams = new URLSearchParams(window.location.search);
      const tabParam = urlParams.get('tab') || urlParams.get('@tab');
      if (tabParam && ['songs', 'albums', 'playlists', 'likes', 'fanlink', 'following'].includes(tabParam.toLowerCase())) {
    ...
    try {
      ...
      switch (tab) {
        ...
        case 'likes':
          ...
          break;
        case 'following':
          endpoint = 'https://singnify.com/api/v2/php/get-user-followed-artists.php';
          break;
              case 'fanlink':
      //    endpoint = 'https://singnify.com/api/v2/php/set-fanlink-form.php';
         //    formData.append("music_id","183");
              endpoint = 'https://singnify.com/api/v2/php/get-user-tracks.php';
      ...
      if (data.status === "200" && data.message === "success") {
        ...
        switch (tab) {
          case 'songs':
            ...
            break;
          case 'albums':
            setAlbums(data.result || data.albums || []);
            break;
          case 'playlists':
            setPlaylists(data.result || data.result || []);
            break;
          case 'likes':
            setLikedItems( data.result || []);
            break;
          case 'following':
            setFollowing(data.result || []);
            break;
             case 'fanlink':
            setFanlink(data.result || []);
            break;
        }
      } else {
    ...
    switch (activeTab) {
      ...
      case 'following':
        return <FollowingTab following={following} loading={loading} />;
            case 'fanlink':
        return <FanlinkTab fanlink={fanlink} loading={loading} />;
      default:
        return (
          <SongsTab 
            tracks={tracks}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex overflow-x-auto">
            {[
             ...
             { id: 'fanlink', label: 'Fanlinks', icon: Share },
