...
const MusicSubmissionForm: React.FC = () => {
  ...
  const nextStep = (): void => {
    ...
    if (currentStep === 1) {
      // Validate submission type selection
      if (!submissionType) {
        isValid = false;
        errorMsg = 'Please select whether you are submitting a Single or Album.';
      }
    } else if (currentStep === 2) {
      // Validate track info based on submission type
      ...
      if (submissionType === 'single') {
        ...
        for (let i = 0; i < tracks.length; i++) {
          if (!tracks[i].audio) {
            isValid = false;
            errorMsg = `Please upload an Audio File for Track ${i + 1}.`;
            break;
          }
        }
      }
          ...
          <div className="space-y-8 animate-fade-in-up">
            ...
            {submissionType === 'single' ? (
              ...
              <>
                ...
                {tracks.map((track, index) => (
                  <div key={track.id} className="bg-slate-800/30 p-4 rounded-lg mb-4">
                    <h4 className="text-white font-medium mb-3">Track {index + 1}: {track.name || 'Untitled'}</h4>
                    ...
                    <div className="space-y-3">
                      ...
                      <TrackUploadComponent
                        ...
                        onError={(error:any) => {
                          console.error('Audio upload error:', error);
                          setErrorMessage(`Audio upload failed for track ${index + 1}: ${error?.message||"Error in upload"}`);
                          setSubmissionStatus('error');
                        }}
                        onSuccess={(fileUrl, response) => {
                          console.log('Audio upload success:', response);
