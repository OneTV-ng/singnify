...
const submitForm = async () => {
    ...
    // Iterate over all keys in formData and append them to FormData object
    (Object.keys(formData) as Array<keyof typeof formData>).forEach(key => {
      const value = formData[key];

      // Special handling for boolean values (is_explicit, is_cover) to send '1' or '0'
      if (key === 'is_explicit' || key === 'is_cover') {
        formDataToSubmit.append(key, value ? '1' : '0');
      }
      // Special handling for File objects (audio, image, lyrics_file)
      else if (value instanceof File) {
        // Only append if a file is actually present
        if (value) {
          formDataToSubmit.append(key, value, value.name);
        }
      }
      // For other fields (strings, numbers, etc.), append if not null, undefined, or empty string
      else if (value !== null && value !== undefined && value !== '') {
        formDataToSubmit.append(key, String(value)); // Convert value to string explicitly
      }
    });

    // Ensure token, track_id, and music_upload_date are explicitly added, even if empty/default
    // These are often API-specific or managed by the backend, but included as per your CURL example.
    formDataToSubmit.append('token', formData.token || ''); // Assuming token might be empty
    formDataToSubmit.append('track_id', formData.track_id || ''); // Assuming track_id might be empty initially
    formDataToSubmit.append('music_upload_date', formData.music_upload_date);

    try {
      // It's good practice to log the FormData contents for debugging before sending
      // For debugging:
      ...
      console.log('API Response:', result); // Log the full API response for inspection

      // Assuming your API returns a JSON object with a 'status' key
      if (response.ok && result.status === 'success') {
        setSubmissionStatus('success');
        // Optionally, reset the form after successful submission
        // setFormData({ /* initial state here */ });
      } else {
        // Handle API errors based on its response structure
        const errorMessage = result.message || 'An unknown error occurred during submission.';
        setSubmissionStatus('error');
        console.error('Submission failed:', errorMessage);
        // You might want to display specific error messages from the API to the user
      }
    } catch (error) {
...
(Object.keys(formData) as Array<keyof typeof formData>).forEach(key => {
      const value = formData[key]; // Now TypeScript knows 'value' corresponds to the type of 'formData[key]'
