...
interface SelectFieldProps extends React.SelectHTMLAttributes<HTMLSelectElement> {
  placeholder: string;
  value: string;
  onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void; // Corrected type
...
const InputField = React.memo((
  { type = 'text', placeholder, value, onChange, error, ...props }: InputFieldProps
) => (
...
const SelectField = React.memo((
  { value, onChange, options, placeholder, error }: SelectFieldProps // Changed to SelectFieldProps
) => (
  <div className="relative">
    <select
      value={value}
...
interface Step {
  ...
  fields: Array<keyof FormData>; // Use keyof FormData for type safety
}
...
return /^[+]?([0-9][0-9]{5,14})$/.test(cleaned) 
  ? '' 
   ...
   */
  const validateStep = useCallback((stepIndex: number): { isValid: boolean, errors: { [key: string]: string } } => {
    const stepFields = steps[stepIndex].fields;
    let isValid = true;
    const errors: { [key: string]: string } = {};

    stepFields.forEach(field => {
      if (field === 'about' || field === 'referralCode' || field === 'recordLabel' || field === 'stageName') {
  ...
  }, [currentStep, currentStepValidation, steps.length]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok && data.status === "200") {
        ...
        showMessage(`🎉 Welcome to Singnify, ${formData.firstName}! Your account has been created successfully! and a welcome message in your email inBox`, "success");
setTimeout(() => {
  router.push('/signin'); // Redirect to welcome page after 3 seconds
},  3000);
        ...
        return (
          <div className="space-y-6 animate-fadeIn">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              />
              <InputField
                placeholder="Stage Name or Alias (Optional)"
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 text-white w-full">
          <div>
            <div className="flex items-center space-x-3 text-3xl font-bold">
              <div className="hidden w-12 h-12 bg-gradient-to-r from-yellow-400 to-yellow-600 rounded-xl flex items-center justify-center shadow-lg">
                <Music className="w-7 h-7 text-black" />
              </div>
              <span className="bg-gradient-to-r from-yellow-400 to-yellow-600 bg-clip-text text-transparent">
               <img  src="https://singnify.com/assets/img/icon-large.png" alt="Singnify"    />  
              </span>
            </div>
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="mt-10 text-center">
            <p className="text-gray-400 text-lg">
              ...
              <a href="/signin" className="text-yellow-400 hover:text-yellow-300 font-semibold transition-colors hover:underline">
                Sign In Here
              </a>
            </p>
          </div>
