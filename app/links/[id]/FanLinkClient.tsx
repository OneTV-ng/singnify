...
const PLATFORMS = [
  ...
  { 
    ...
    key: "singnify", 
    name: "Singnify", 
    logo: "https://app.singnify.com/images/icon-large.png",
    color: "bg-[#3668ff]"
  },
];
...
export default function FanLinkClient({ songData, artist, title, id=""
 }: FanLinkClientProps) {
  const [showVideo, setShowVideo] = useState(false);
  const [dominantColor, setDominantColor] = useState("59, 130, 246");

  const { spotify, fanlinks } = songData;
  const coverImage = spotify?.album?.image || songData.album_cover || "https://app.singnify.com/assets/default-cover.jpg";
  ...
  // Get Spotify URI for embed
  const spotifyUri = spotify?.uri || fanlinks?.spotify;
  const spotifyId = spotifyUri 
    ? spotifyUri.includes("track/") 
      ? spotifyUri.split("track/")[1]?.split("?")[0]
...
<li 
                ...
                >
                  <a 
                    href={`https://app.singnify.com/song/${id}`}
                    target="_blank" 
  ...
  src="https://app.singnify.com/images/icon-large.png"
  alt={`Singnify Logo`}
  width={0}               // allows Next.js to infer width dynamically
  height={30}    
    unoptimized         // fixed height of 30px
