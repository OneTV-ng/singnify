...
const YOUTUBE_API_KEY = process.env.YOUTUBE_API_KEY!; // optional (for more accurate results)
...
// 🔹 Helper: Get YouTube Music Info (optional, uses YouTube Data API)
async function getYouTubeMusicLink(artist: string, title: string) {
  if (!YOUTUBE_API_KEY) {
    // fallback heuristic link
  ...
  const res = await fetch(url);
  if (!res.ok) throw new Error("Failed to get YouTube data");
