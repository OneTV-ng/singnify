...
export default function Sidebar() {
 ...
 // import { useSession } from "next-auth/react";
  ...
  const artistMenu =(user?.IsArtist==="1")?
  
 [{ name: 'Artist Dashboard', href: '/artist/', icon: Mic2, childOf: 'Search'},
    { name: 'Distribute', href: '/distribute', icon: Library },
    { name: 'Music Upload', href: '/artist/upload', icon: Upload },]
  :
  [{ name: 'Library', href: '/library', icon: Library, childOf: 'Search'}, { name: 'Connect', href: '/connect', icon: Share, childOf: 'Search'}, { name: 'Playlists', href: '/playlists', icon: Music , childOf: 'Search'}]
 ;
  useEffect(() => {
    const checkMobile = () => {
