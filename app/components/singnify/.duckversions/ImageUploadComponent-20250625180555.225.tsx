...
console.log("API KET =", PLATFORM_API_KEY);
...
interface ImageUploadProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'onChange' | 'value' | 'type' | 'onError'> {
  ...
  required?: boolean;
  multiple?: boolean;
  // New prop for authentication method
...
const ImageUploadComponent: React.FC<ImageUploadProps> = ({
  ...
  required = false,
  multiple = false,
  authMethod = 'body', // Default to body to avoid CORS issues
  ...props
}) => {
  ...
  const uploadFile = async (file: File): Promise<void> => {
    try {
      ...
      if (authMethod === 'query') {
        ...
        requestBody = JSON.stringify({ image: base64 });
      } else if (authMethod === 'body') {
        // Add API key in request body
        requestBody = JSON.stringify({ 
          image: base64,
          api_key: apiKey 
        });
      } else {
        // Try with Authorization header (original method)
        if (apiKey) {
  ...
  const handleFileSelect = useCallback((files: FileList | null) => {
    ...
    uploadFile(file);
  }, [maxSize, minWidth, maxWidth, minHeight, maxHeight, showPreview, apiEndpoint, apiKey, onChange, onSuccess, onError, authMethod]);
