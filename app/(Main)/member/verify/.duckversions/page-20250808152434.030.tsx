...
function VerificationForm() { 
  ...
  // Changed to a function declaration for robust compatibility
...
// Function to collect social media URLs
  ...
  const prefillFormData = (userData: ExtendedUserType) => {
    ...
    setFormData(prev => ({
      ...
      picture: userData.Gender || '',
      country: userData.Picture || '',
  ...
  // Handle input changes
  const xhandleInputChange = (field: string, value: string) => {
    setFormData(prev => ({ ...prev, [field]: value }));
    // Clear validation error when user starts typing
    if (validationErrors[field]) {
      setValidationErrors(prev => ({ ...prev, [field]: '' }));
  ...
  const apiUrl = 'https://singnify.com/api/v2/php/update-profile.php';
  ...
  payload.append('uname', formData.uname);
  payload.append('gender', formData.gender);
  payload.append('phone', formData.phone);
  payload.append('country', formData.country);
  payload.append('image', formData.image); // URL or file
  ...
  try {
    ...
    if (result.status === "200") {
      ...
      responseMessage.innerHTML = `
        <div class="bg-green-800 p-6 rounded-lg shadow-xl text-white text-center">
          <p class="text-xl font-bold mb-4">Profile updated successfully!</p>
          <button class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg mt-4" onclick="this.parentNode.parentNode.remove()">Close</button>
        </div>
      `;
    ...
    errorMessage.innerHTML = `
      <div class="bg-red-800 p-6 rounded-lg shadow-xl text-white text-center">
        <p class="text-xl font-bold mb-4">Update failed: ${error.message}</p>
...
const handleSubmit = async () => {
  ...
  try {
    const response = await fetch(apiUrl, {
      ...
      headers: {
        // Remove any custom headers that might interfere with FormData
      }
    });
    ...
    errorMessage.innerHTML = `
      <div class="bg-gray-800 p-6 rounded-lg shadow-xl text-white text-center max-w-md">
        ...
        <p class="text-gray-300 mb-4">${error?.message as string || 'An error occurred while submitting your verification. Please try again.'}</p>
...
const handleSubmitxx = async () => {
  if (!validateStep(currentStep)) return;

  setIsSubmitting(true);

  const apiUrl = 'https://singnify.com/api/v2/php/submit-verification.php?API_KEY=7c6a180b36896a0a8c02787eeafb0e4c';
...
/>
      ...
      />
            ...
            // NEW: Structured bank details component
